    ...
    <div className="min-h-screen bg-[#0b0c10] text-[#1e2022] pb-32 font-sans">
            ...
            {filteredProducts.map((item) => (
              <div key={item.id} className="bg-[#f9f9f7] rounded-2xl p-4 flex flex-col justify-between border border-stone-200/60 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.2)] transition-all duration-300 hover:shadow-[0_20px_35px_-5px_rgba(0,0,0,0.35)] hover:-translate-y-0.5">
                <div>
                  ...
                  <h3 className="font-serif italic text-base text-zinc-900 mt-4 font-bold tracking-tight line-clamp-1">
                    {getProductName(item.name)}
                  </h3>
                  <p className="text-[11px] text-stone-500 mt-1 leading-normal line-clamp-2">
      ...
      {totalItems > 0 && (
        <div className="fixed bottom-6 left-4 right-4 z-50 max-w-sm mx-auto">
          <button onClick={handleSendOrder} className="w-full py-4 bg-zinc-950 text-stone-100 font-medium rounded-2xl text-center text-sm tracking-wider uppercase transition-all duration-300 active:scale-98 border border-zinc-800 shadow-[0_20px_40px_rgba(0,0,0,0.7)] hover:bg-rose-800 flex items-center justify-center gap-3 cursor-pointer"><ShoppingBag className="w-4 h-4" />{t.buyurtma} ({totalItems})</button>
        </div>
      )}
